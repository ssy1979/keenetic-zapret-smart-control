...
auto_filter_opts(){ echo '--hostlist=/example/auto --hostlist-auto=/example/auto'; }
...
[ "$mode_args" = '--filter-tcp=80 --lua-desync=multisplit --hostlist=/example/auto --hostlist-auto=/example/auto --new=tls --filter-tcp=443 --lua-desync=multisplit --hostlist=/example/auto --hostlist-auto=/example/auto --new' ] || fail 'Auto mode lost a profile or applied hostlists across boundaries'
...
(
  ...
  chown(){ return 1; }
  if profile_with_mode PPPoE1 '--lua-desync=multisplit' >/dev/null 2>&1; then
