...
write_release v1.0.0-generic attacker
...
grep -Fq '[ -f "$SRC/opt/kzsc/www/cgi-bin/$b" ] && continue' "$SRC/install.sh" || fail "installer does not retain all shipped CGI endpoints"
grep -Fq '[ -x "/opt/kzsc/www/cgi-bin/$b" ] ||' "$SRC/install.sh" || fail "installer CGI executable postcondition missing"
