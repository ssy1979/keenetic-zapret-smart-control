...
grep -Fq 'pppoe_connect_via(){' "$ROOT/opt/kzsc/bin/kzsc-dns.sh" || die 'PPPoE reconnect helper is missing'
grep -Fq 'interface $nd connect via $via' "$ROOT/opt/kzsc/bin/kzsc-dns.sh" || die 'PPPoE DNS renewal fallback is missing'
