...
grep -Fq 'interface $nd $client no name-servers' "$ROOT/opt/kzsc/bin/kzsc-dns.sh" || die 'canonical ISP DNS ignore command is missing'
grep -Fq 'add_fallback_dns(){' "$ROOT/opt/kzsc/bin/kzsc-dns.sh" || die 'Cloudflare fallback is missing'
grep -Fq 'ip name-server 1.1.1.1' "$ROOT/opt/kzsc/bin/kzsc-dns.sh" || die 'Cloudflare primary fallback is missing'
grep -Fq 'ip name-server 1.0.0.1' "$ROOT/opt/kzsc/bin/kzsc-dns.sh" || die 'Cloudflare secondary fallback is missing'
