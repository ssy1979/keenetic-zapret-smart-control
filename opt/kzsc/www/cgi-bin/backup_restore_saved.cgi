...
[ -s "/opt/kzsc/var/backups/$name" ] || { echo '{"ok":false,"error":"backup_not_found"}'; exit 0; }
rid="backup-restore-saved-$(date +%s)-$$"; req="/tmp/kzsc-backup-req.$rid"; printf '%s\n' "backup_restore_saved:$name" >"$req" || { rm -f "$req"; echo '{"ok":false,"error":"queue_write_failed"}'; exit 0; }; printf '{"ok":true,"request_id":"%s"}\n' "$rid"
