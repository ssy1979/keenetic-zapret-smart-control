...
valid_id(){ case "$1" in kablonet|kzm2-*) [ -f "$DIR/$1.conf" ];; auto_*) [ -f "$AUTO/$1.conf" ];; *) return 1;; esac; }
...
write_json(){
  ...
  tmp="$OUT.tmp.$$"
  {
    printf '{"presets":['
    first=1
    for id in kablonet $(find "$DIR" -maxdepth 1 -type f -name 'kzm2-*.conf' 2>/dev/null | sed 's#.*/##;s/\.conf$//' | sort) $(find "$AUTO" -maxdepth 1 -type f -name 'auto_*.conf' 2>/dev/null | sed 's#.*/##;s/\.conf$//' | sort); do
      nm="$(field "$id" NAME)"
      src="$(field "$id" SOURCE)"
