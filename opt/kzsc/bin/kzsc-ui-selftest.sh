...
grep -Fq 'Profile selection is based on HTTP + HTTPS/TLS reachability, not the ISP label.' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq '[ "$state" = success ] && [ "$rc" = 0 ]' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq 'kzsc-engines.sh reconfigure "$nd"' /opt/kzsc/bin/kzsc-blockcheck.sh && ok "Blockcheck canlı hazır profil doğrulaması / hızlı işçi yarışı" || bad "Blockcheck canlı hazır profil doğrulaması / hızlı işçi yarışı"
