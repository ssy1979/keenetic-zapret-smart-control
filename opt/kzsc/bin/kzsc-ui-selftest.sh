...
grep -Fq 'Profile selection requires HTTPS reachability; plain HTTP is recorded separately and the ISP label is not used.' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq '[ "$https_ok" -eq 1 ]' /opt/kzsc/bin/kzsc-blockcheck.sh && ! grep -Fq 'Saved profile is active:' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq '[ "$state" = success ] && [ "$rc" = 0 ]' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq 'kzsc-engines.sh reconfigure "$nd"' /opt/kzsc/bin/kzsc-blockcheck.sh && ok "Blockcheck canlı HTTPS hazır profil doğrulaması / hızlı işçi yarışı" || bad "Blockcheck canlı HTTPS hazır profil doğrulaması / hızlı işçi yarışı"
