...
ce "$CGI/zapret2_check.cgi" "Zapret2 Sürüm Kontrolü"
ce "$CGI/zapret2_status.cgi" "Zapret2 durum"
ce "$CGI/zapret2_repair.cgi" "Zapret2 Onar"
ce "$CGI/zapret2_stop.cgi" "Zapret2 Durdur"
ce "$CGI/zapret2_start.cgi" "Zapret2 Başlat"
ce "$CGI/zapret2_remove.cgi" "Zapret2 Kaldır"
ce "$CGI/zapret2_ipv6.cgi" "Zapret2 IPv6"
ce "$CGI/zapret2_update_auto.cgi" "Zapret2 otomatik güncelleme"
grep -Fq 'kzsc_safe_repair(){' /opt/kzsc/bin/kzsc-lib.sh && ok "Güvenli otomatik runtime onarımı" || bad "Güvenli otomatik runtime onarımı"
...
grep -Fq 'KZSC politika servisi bulunamadı' "$WWW/index.html" && ok "DPI politika HTML/JSON hata ayrımı" || bad "DPI politika HTML/JSON hata ayrımı"
grep -Fq 'Henüz rezervasyon yok' "$WWW/index.html" && grep -Fq 'WAN DPI Profili' "$WWW/index.html" && ok "Cihaz Zapret/rezervasyon arayüz açıklığı" || bad "Cihaz Zapret/rezervasyon arayüz açıklığı"
...
for nd in $(internet_wans); do
 ...
 for q in kablonet $(find "$KZSC_HOME/share/dpi-presets" -maxdepth 1 -type f -name '*.conf' 2>/dev/null | sed 's#.*/##;s/\.conf$//' | sort); do ce "$CGI/profile_set_${id}_${q}.cgi" "$nd Profil $q"; done
done
...
grep -Fq "kzscRestartBtn')?.addEventListener" "$WWW/index.html" && grep -Fq 'waitKzscServiceReady' "$WWW/index.html" && grep -Fq 'ACTION="restart"' "$CGI/restart.cgi" && grep -Fq '"$RID" "$ACTION"' "$CGI/restart.cgi" && grep -Fq '[ "$action" = "restart" ]' /opt/kzsc/bin/kzsc-maintenance.sh && ok "KZSC yeniden başlatma UI/CGI/bakım akışı" || bad "KZSC yeniden başlatma akışı"
grep -Fq "routerRebootBtn')?.addEventListener" "$WWW/index.html" && grep -Fq "'X-KZSC-Action':'router-reboot'" "$WWW/index.html" && grep -Fq 'REQUEST_METHOD:-GET' "$CGI/router_reboot.cgi" && grep -Fq 'HTTP_X_KZSC_ACTION' "$CGI/router_reboot.cgi" && grep -Fq 'ACTION="router_reboot"' "$CGI/router_reboot.cgi" && grep -Fq '"$RID" "$ACTION"' "$CGI/router_reboot.cgi" && grep -Fq "system reboot 30" /opt/kzsc/bin/kzsc-maintenance.sh && grep -Fq 'for candidate in /bin/ndmc' /opt/kzsc/bin/kzsc-maintenance.sh && grep -Fq "router_reboot) cat=system; title='Router Yeniden Başlatma'" /opt/kzsc/bin/kzsc-telegram.sh && ok "Router yeniden başlatma UI/POST/CGI/NDMC/Telegram akışı" || bad "Router yeniden başlatma akışı"
...
grep -q '^MAX_SECONDS=0$' /opt/kzsc/bin/kzsc-blockcheck.sh && ! grep -q '^KZSC_BLOCKCHECK_MAX_SECONDS=' /opt/kzsc/etc/kzsc.conf && ok "Blockcheck sabit süre sınırı yok" || bad "Blockcheck sabit süre sınırı yok"
...
grep -q 'cleanup_job_children' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -q 'cleanup_temp_chains' /opt/kzsc/bin/kzsc-blockcheck.sh && ok "Blockcheck stop process-tree temizliği" || bad "Blockcheck stop process-tree temizliği"
...
grep -q '^ipv6_runtime_probe(){' /opt/kzsc/bin/kzsc-native-dpi.sh && grep -q 'önceki güvenli durum geri yüklendi' /opt/kzsc/bin/kzsc-native-dpi.sh && ok "IPv6 runtime probe ve rollback" || bad "IPv6 runtime probe ve rollback"
grep -q 'Zapret2 IPv6 desteği' /opt/kzsc/www/index.html && grep -q 'Zapret2 Sürümünü Kontrol Et' /opt/kzsc/www/index.html && grep -q 'Zapret2 IPv6 support' /opt/kzsc/www/index.html && ok "Zapret2 IPv6 TR/EN çevirileri ve sürüm kontrolü" || bad "Zapret2 IPv6 TR/EN çevirileri ve sürüm kontrolü"
...
grep -q '^prepare_quick_testset(){' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq 'export TEST_DEFAULT="$testset"' /opt/kzsc/bin/kzsc-blockcheck.sh && ok "Blockcheck kısa strateji seti / süre sınırı yok" || bad "Blockcheck kısa strateji seti / süre sınırı yok"
...
grep -Fq 'Fast profile selection requires HTTPS reachability; plain HTTP is skipped and the ISP label is not used.' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq 'export ENABLE_HTTPS_TLS13=1' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq 'export SKIP_DNSCHECK=1' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq 'export TEST_DEFAULT="$testset"' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq '[ "$https_ok" -eq 1 ]' /opt/kzsc/bin/kzsc-blockcheck.sh && ! grep -Fq 'Saved profile is active:' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq '[ "$state" = success ] && [ "$rc" = 0 ]' /opt/kzsc/bin/kzsc-blockcheck.sh && grep -Fq 'kzsc-engines.sh reconfigure "$nd"' /opt/kzsc/bin/kzsc-blockcheck.sh && ok "Blockcheck en kısa HTTPS doğrulaması / hızlı işçi yarışı" || bad "Blockcheck en kısa HTTPS doğrulaması / hızlı işçi yarışı"
grep -q '^auto_filter_opts(){' /opt/kzsc/bin/kzsc-native-dpi.sh && grep -Fq -- '--hostlist-auto-fail-threshold=3' /opt/kzsc/bin/kzsc-native-dpi.sh && ! grep -q '^auto_filter_global_opts(){' /opt/kzsc/bin/kzsc-native-dpi.sh && ok "Otomatik hostlist seçenekleri filtre profillerinde" || bad "Otomatik hostlist seçenekleri filtre profillerinde"
grep -Fq 'motorun etkin durumu korundu ve otomatik onarım yeniden deneyecek' /opt/kzsc/bin/kzsc-native-dpi.sh && grep -Fq 'önceki ${old_mode} modu ve motor durumu geri yüklendi' /opt/kzsc/bin/kzsc-maintenance.sh && ok "Çalışma modu motor durumunu korur / geri alma" || bad "Çalışma modu motor durumunu korur / geri alma"
...
grep -Fq 'function noticeTranslatedText' /opt/kzsc/www/index.html && grep -Fq 'function localeDate' /opt/kzsc/www/index.html && ok "Görünür UI çift dilli mesaj/tarih yardımcıları" || bad "Görünür UI çift dilli mesaj/tarih yardımcıları"
...
grep -q '^source_retired_scan(){' /opt/kzsc/bin/kzsc-purity.sh && grep -q '^required_payload_check(){' /opt/kzsc/bin/kzsc-purity.sh && ok "KZSC kaynak referansı ve eksiksiz paket denetimi" || bad "KZSC kaynak/paket denetimi"
...
grep -q '^external_check(){' /opt/kzsc/bin/kzsc-purity.sh && grep -Fq 'read-only' /opt/kzsc/bin/kzsc-purity.sh && ok "Harici kalıntı bildirimi salt okunur" || bad "Harici kalıntı salt okunur denetimi"
...
grep -q 'function notifyKzscUpdateTransition' "$WWW/index.html" && grep -q "showGlobalNotice('success'.*KZSC güncellemesi başarıyla kuruldu" "$WWW/index.html" && grep -q 'Güncelleme sonrası sayfayı yenileyin' "$WWW/index.html" && grep -q 'Refresh the page after the update' "$WWW/index.html" && ok "KZSC Güncelleme üst bildirimleri" || bad "KZSC Güncelleme üst bildirimleri"
