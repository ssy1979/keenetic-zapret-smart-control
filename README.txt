...
v1.0.0-generic / Windows Hazırlayıcı 1.0.0
...
v1.0.0-generic operating-mode input and audit fixes
...
Standart kurulum: `cd /opt/tmp && sha256sum -c keenetic-zapret-smart-control-v1.0.0-generic.tar.gz.sha256 && tar -xzf keenetic-zapret-smart-control-v1.0.0-generic.tar.gz && cd keenetic-zapret-smart-control-v1.0.0-generic && sh install.sh`
...
Keenetic Zapret Smart Control (KZSC)
v1.0.0-generic daemon lifecycle and stale PID recovery
...
Keenetic Zapret Smart Control (KZSC)
v1.0.0-generic KZSC/router restart controls and web UI cleanup
...
Keenetic Zapret Smart Control (KZSC)
v1.0.0-generic BusyBox updater scope fix and Telegram update controls
...
- v1.0.0-generic/v1.0.0-generic güncelleyicisi kendi hatasını otomatik düzeltemediği için v1.0.0-generic bir kez elle kurulmalıdır; sonraki sürümler otomatik alınabilir.

Standart kurulum: `cd /opt/tmp && sha256sum -c keenetic-zapret-smart-control-v1.0.0-generic.tar.gz.sha256 && tar -xzf keenetic-zapret-smart-control-v1.0.0-generic.tar.gz && cd keenetic-zapret-smart-control-v1.0.0-generic && sh install.sh`


Keenetic Zapret Smart Control (KZSC)
v1.0.0-generic web update queue permission hotfix
...
Standart kurulum: `cd /opt/tmp && tar -xzf keenetic-zapret-smart-control-v1.0.0-generic.tar.gz && cd keenetic-zapret-smart-control-v1.0.0-generic && sh install.sh`


Keenetic Zapret Smart Control (KZSC)
v1.0.0-generic adaptive Keenetic compatibility and absolute Blockcheck deadline
...
Keenetic Zapret Smart Control (KZSC)
v1.0.0-generic Blockcheck endpoint race fix
...
Standart kurulum: `tar -xzf keenetic-zapret-smart-control-v1.0.0-generic.tar.gz && cd keenetic-zapret-smart-control-v1.0.0-generic && sh install.sh`

Eski ürün kalıntısı uyarısı alınırsa ve o ürünü artık kullanmadığınızdan eminseniz: `sh install.sh --remove-retired`


Keenetic Zapret Smart Control (KZSC)
v1.0.0-generic reliability, translation and backup hardening
...
- Release metadata ve audit/self-test kontrolleri v1.0.0-generic için güncellendi.
...
Keenetic Zapret Smart Control (KZSC)
v1.0.0-generic Overview WAN reconcile UI cleanup
...
- v1.0.0-generic Telegram config/runtime state ayrımı, backup hardening ve audit kontrolleri aynen korunur.
- Canlı doğrulama: UI self-test RC=0, runtime audit RC=0; “Genel Bakış WAN Reconcile kartı gizli / backend aktif” kontrolü OK.


Keenetic Zapret Smart Control (KZSC)
v1.0.0-generic Telegram runtime state separation
...
- İlk çalıştırmada v1.0.0-generic ve daha eski `telegram.conf` içindeki runtime değerleri kaybedilmeden yeni state dosyasına migrate edilir ve config normalize edilir.
...
- v1.0.0-generic version consistency guard ve v1.0.0-generic HTTP/CGI runtime audit hardening aynen korunur.
- Canlı doğrulama: backup davranış testi, config hash stabilitesi, self-test, runtime, version, HTTP ve full audit RC=0; diagnostics v1.0.0-generic.


Keenetic Zapret Smart Control (KZSC)
v1.0.0-generic release metadata consistency guard

=== v1.0.0-generic ===
- v1.0.0-generic paketindeki aktif sürüm metadata referanslarının v1.0.0-generic'de kalması düzeltildi.
...
- Farklı/eski aktif `v1.0.0-generic.x-generic` referansı kalırsa audit artık FAIL üretir.
- v1.0.0-generic HTTP/CGI runtime audit hardening aynen korunur.
- Canlı doğrulama: version/http/full audit RC=0; diagnostics v1.0.0-generic; Settings, KeenDNS ve servisler sağlıklı.


Keenetic Zapret Smart Control (KZSC)
v1.0.0-generic HTTP/CGI runtime audit hardening
...
v1.0.0-generic final ownership/audit hardening
- Copy/Open KeenDNS UI actions are operation-log audited (retained from v1.0.0-generic).
...
Keenetic Zapret Smart Control (KZSC) v1.0.0-generic
...
v1.0.0-generic sağlamlaştırma / audit
...
- v1.0.0-generic'dan yükseltmede mevcut validated kayıtları ve Olay Günlüğü kullanılarak görünürlük geçmişi otomatik bootstrap edilir.
...
- v1.0.0-generic'ın WAN topology reconcile, preset-first doğrulama, broad Blockcheck/AUTO fallback ve reboot persistence davranışları korunur.
...
- v1.0.0-generic'teki kısa dosya adıyla backup restore davranışı korunur.
...
v1.0.0-generic Blockcheck stabilization
...
v1.0.0-generic preset-first Blockcheck
...
v1.0.0-generic runtime cleanup/reconcile
...
v1.0.0-generic Telegram polish
...
v1.0.0-generic Telegram callback authorization fix
...
v1.0.0-generic: Final TR/EN UI translation audit; explicit bilingual global operation notices in English mode; Event Log dynamic message/date localization; missing Overview/KeenDNS/Backup translations completed.


v1.0.0-generic: KeenDNS Copy Address now works on the local HTTP panel using a clipboard fallback, with visible TR/EN success/error notices.
