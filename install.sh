...
# v1.0.6-generic: clean orphaned upstream Blockcheck children left by older builds.
...
# v1.0.6-generic: Blockcheck uses the shortest strategy family set without an
...
# v1.0.6-generic: scheduled nightly Blockcheck is disabled by default. Manual Blockcheck + auto-apply remains enabled.
...
# First v1.0.6-generic installation adopts the already-working WANs as baseline so an
...
printf '%s\n' '1.0.6-generic' >/opt/kzsc/var/update/latest
printf '%s\n' 'https://github.com/ssy1979/keenetic-zapret-smart-control/releases/tag/v1.0.6-generic' >/opt/kzsc/var/update/release_url
...
ROLLBACK_ARMED=0
[ -z "$UPGRADE_BACKUP" ] || rm -rf "$UPGRADE_BACKUP"
echo "Keenetic Zapret Smart Control v1.0.6-generic kuruldu."
